...
public class HBAO_Core : MonoBehaviour
{
    ...
    protected static class Pass
    {
        ...
        public const int Debug_ViewNormals = 47;
    }
    ...
    private static readonly int[] numSampleDirections = new int[] { 3, 4, 6, 8, 8 }; // LOWEST, LOW, MEDIUM, HIGH, HIGHEST (highest uses more steps)
    ...
    protected void UpdateShaderProperties()
    {
        ...
        _renderTarget.height = _hbaoCamera.pixelHeight;
#endif
        _renderTarget.downsamplingFactor = generalSettings.resolution == Resolution.Full ? 1 : 2;
        ...
        float maxRadInPixels = Mathf.Max(16, aoSettings.maxRadiusPixels * Mathf.Sqrt((_renderTarget.width * _renderTarget.height) / (1080.0f * 1920.0f)));
        ...
        else
        {
            ...
            if (generalSettings.resolution == Resolution.Half)
        ...
        if (noiseTex == null || _quality != generalSettings.quality)
        {
            if (noiseTex != null)
                DestroyImmediate(noiseTex);
        ...
        _hbaoMaterial.SetFloat(ShaderProperties.maxRadiusPixels, maxRadInPixels / _renderTarget.deinterleavingFactor);
    ...
    {
        _hbaoShaderKeywords[0] = colorBleedingSettings.enabled ? "COLOR_BLEEDING" : "__";

        if (_renderTarget.orthographic)
            _hbaoShaderKeywords[1] = "ORTHOGRAPHIC_PROJECTION";
        else
            _hbaoShaderKeywords[1] = IsDeferredShading() ? "DEFERRED_SHADING" : "__";
    ...
    {
        noiseTex = new Texture2D(4, 4, TextureFormat.RGB24, false, true);
        ...
        for (int x = 0; x < 4; ++x)
        {
            for (int y = 0; y < 4; ++y)
            {
                ...
                float angle = 2.0f * Mathf.PI * r1 / numSampleDirections[GetAoPass()];
    ...
    public void ApplyPreset(Preset preset)
    {
        ...
        switch (preset)
        {
            ...
            case Preset.HighestQuality:
                ...
                m_BlurSettings.type = Blur.Narrow;
                break;
            case Preset.Normal:
            default:
                break;
